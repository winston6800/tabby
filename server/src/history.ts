 ...
 */
function addToHisotry(userId:number, nodeId: number, title: string, description: string, expectedOutput: string, timeSpent: string, tags: string, callback: (err: Error | null, result?: any) => void) {
  const sql = `INSERT INTO edges (userId, nodeId, title, description, expectedOutput, timeSpent, tags) VALUES (?, ?, ?, ?, ?, ?, ?)`;

  db.run(sql, [userId, nodeId, title, description, expectedOutput, timeSpent, tags], function (err: Error | null) {
 ...
 */
function getHistory(userId: number | null, callback: (err: Error | null, rows?: any[]) => void): void {
  const sql = userId
